...
if [ -n "${RV_INSTALLER_GHE_BASE_URL:-}" ]; then
    ...
    INSTALLER_BASE_URL="${RV_INSTALLER_GITHUB_BASE_URL:-https://github.com}"
fi
if [ -n "${RV_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URL="$RV_DOWNLOAD_URL"
...
if [ -n "${RV_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$RV_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${RV_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$RV_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${RV_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${RV_UNMANAGED_INSTALL:-}"
...
RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/rv"
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set RV_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="rv-aarch64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="rv-x86_64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
install() {
    ...
    if [ -n "${RV_INSTALL_DIR:-}" ]; then
        _force_install_dir="$RV_INSTALL_DIR"
